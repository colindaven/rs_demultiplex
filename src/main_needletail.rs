...
//fn write_fastq_record(writer: Write, rec: &SequenceRecord) {
...
fn main() {
    ...
    while let Some(record) = reader.next() {
        ...
        if sequenceOligo == barcodes_vector[0]{
            ...
            //write_fastq(seqrec.id(), &seqrec.seq(), seqrec.qual(), &mut writer, LineEnding::Unix).expect("Failed to write record.");
            
            //write_fastq_record(file, &seqrec);
            &writer.write_record(seqrec);
        }  
